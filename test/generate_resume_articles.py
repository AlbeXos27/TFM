...
# 2. Claves adaptadas incluyendo métricas de rendimiento
METRICS_KEYS = [
    "final_score",
    "title",
    "autores",
...
# Headers correspondientes para las tablas
METRICS_HEADERS = [
    "Final",
    "Título",
    "Autores",
...
def load_dataset_from_folder(dataset_dir: Path, origen_label: str) -> list[dict]:
    ...
    for model_name in MODEL_FOLDERS:
        json_path = (
            dataset_dir
            / model_name
            / f"comparison_vs_{REFERENCE_MODEL}_articles.json"
        )
        ...
        for res in data.get("results", []):
            ...
            # Incorporación de métricas de tiempo y tokens desde el nivel raíz o desde details
            combined_metrics = dict(details)
            combined_metrics["tiempo_inferencia"] = res.get("tiempo_inferencia", details.get("tiempo_inferencia"))
            combined_metrics["tokens_generados"] = res.get("tokens_generados", details.get("tokens_generados"))
            combined_metrics["tokens_por_segundo"] = res.get("tokens_por_segundo", details.get("tokens_por_segundo"))

            if archivo:
                if archivo not in files_data:
                    files_data[archivo] = {}
                files_data[archivo][model_name] = combined_metrics
...
def compute_model_averages(datasets_data: list[dict]) -> dict[str, dict[str, float]]:
    """Calcula la media exacta de cada métrica para cada modelo."""
    model_totals = {m_key: {k: 0.0 for k in METRICS_KEYS} for m_key in MODEL_FOLDERS}
    model_counts = {m_key: {k: 0 for k in METRICS_KEYS} for m_key in MODEL_FOLDERS}
        ...
        for model_key in MODEL_FOLDERS:
            if model_key in models_dict:
                ...
                for m_key in METRICS_KEYS:
                    val = details.get(m_key)
                    if val is not None:
                        model_totals[model_key][m_key] += float(val)
                        model_counts[model_key][m_key] += 1

    model_averages = {}
    for model_key in MODEL_FOLDERS:
        model_averages[model_key] = {}
        for m_key in METRICS_KEYS:
            count = model_counts[model_key][m_key]
            if count > 0:
                model_averages[model_key][m_key] = model_totals[model_key][m_key] / count
            else:
                model_averages[model_key][m_key] = 0.0

    return model_averages


def format_value(m_key: str, val: float | None) -> str:
    """Formatea los números para su correcta presentación en la tabla."""
    if val is None:
        return "N/A"
    if m_key == "tokens_generados":
        return f"{val:.0f}"
    return f"{val:.2f}"
...
def export_files_table_image(
    ...
    for item in datasets_data:
        ...
        for m in METRICS_KEYS:
            vals = [
                ...
                if m_key in models_dict and models_dict[m_key].get(m) is not None
            ]
        ...
        for idx, model_key in enumerate(MODEL_FOLDERS):
            ...
            for col_idx, m_key in enumerate(METRICS_KEYS):
                val = details.get(m_key)
                row.append(format_value(m_key, val))

                if max_per_metric[m_key] is not None and val == max_per_metric[m_key] and val > 0:
    ...
    fig, ax = plt.subplots(
        figsize=(26 if include_origen_col else 24, len(table_data) * 0.45 + 1.2), dpi=300
    )
    ax.axis("off")
...
def export_averages_table_image(
    ...
    """Genera la imagen PNG EXCLUSIVA de la TABLA DE PROMEDIOS POR MODELO."""
    if not datasets_data:
        return

    model_averages = compute_model_averages(datasets_data)
    
    headers = ["Modelo Evaluado"] + METRICS_HEADERS

    max_avg_per_metric = {}
    for m in METRICS_KEYS:
        vals = [model_averages[m_key][m] for m_key in MODEL_FOLDERS if model_averages[m_key].get(m) is not None]
    ...
    for row_index, model_key in enumerate(MODEL_FOLDERS):
        ...
        for col_idx, m_key in enumerate(METRICS_KEYS):
            val = avg_details.get(m_key)
            row.append(format_value(m_key, val))

            # +1 de desfase por la columna 'Modelo Evaluado'
            if max_avg_per_metric[m_key] is not None and val == max_avg_per_metric[m_key] and val > 0:
                bold_cells.append((row_index, col_idx + 1))

        table_data.append(row)

    # Crear figura adaptada a la cantidad de columnas (14 métricas + 1 etiqueta)
    fig, ax = plt.subplots(figsize=(18, 3.2), dpi=300)
    ax.axis("off")
...
def main():
    ...
    for dataset_dir, origen_label in DATASET_SOURCES:
        ...
        if folder_data:
            ...
            out_files_png = dataset_dir / "tabla_comparativa_archivos_articles.png"
            ...
            out_avg_png = dataset_dir / "tabla_promedios_modelos_articles.png"
